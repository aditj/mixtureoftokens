...
def generate_with_embedding_mixture(
    ...
    *,
    ...
    Args:
        ...
        layer_offset: Layer offset from the last layer (0 = last layer, 1 = second-to-last, etc.)
    ...
    # Get the final layer (lm_head) for logit lens
    ...
    with torch.no_grad():
        outputs = model(input_ids=input_ids, use_cache=True, return_dict=True, output_hidden_states=True)
    ...
    for round_idx in range(T_e):
        # Get hidden states from the specified layer offset
        hidden_states = outputs.hidden_states  # List of hidden states from each layer
        target_layer_idx = len(hidden_states) - 1 - layer_offset  # 0-indexed from the end
        
        # Ensure we don't go out of bounds
        target_layer_idx = max(0, min(target_layer_idx, len(hidden_states) - 1))
        
        # Debug info for first round only
        if round_idx == 0:
            print(f"Model has {len(hidden_states)} layers, using layer {target_layer_idx} (offset {layer_offset})")
        
        # Get hidden states from the target layer
        target_hidden_states = hidden_states[target_layer_idx][:, -1, :]  # [batch_size, hidden_size]
        
        # Apply logit lens: pass through the final layer to get logits
        last_logits = lm_head(target_hidden_states).squeeze(0)  # [vocab_size]
...
def main():
    ...
    parser.add_argument('--layer_offset', type=int, default=0, help='Layer offset from the last layer (0 = last layer, 1 = second-to-last, etc.)')
    ...
    print(f"T_e: {T_e}, k: {k}, T_total: {T_total}, num_examples: {num_examples}, temperature: {temperature}, layer_offset: {layer_offset}")
    # ---------------------
    ...
    filename = f'generation_comparison/{experiment_name}/{T_total}_{num_examples}_{temperature}/generation_comparison_T_e_{T_e}_k_{k}_layer_offset_{layer_offset}.json'
